...
router.get("/:name", async (req, res) => {
  try {
    ...
    const attributes = await atributeService.getByName(name);
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await atributeService.insertValue(id, valueToAdd);
    ...
    console.error("Error en PUT /atributes:", error);
    res
      .status(500)
      .json({ error: "Hubo un problema interno al procesar la solicitud." });
  }
